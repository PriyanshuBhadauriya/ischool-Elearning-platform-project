...
if(isset($_REQUEST['requpdate'])){
	//checkig for empty fields
    if(($_REQUEST['course_id']=="") || ($_REQUEST['course_disc']=="") || ($_REQUEST['course_author']=="") || ($_REQUEST['course_duration']=="") || ($_REQUEST['course_price']=="") || ($_REQUEST['course_original_price']==""  )|| ($_REQUEST['course_name'] ==""))
	{
		...
		$sql="UPDATE  course SET  course_id = '$cid' , course_name = '$cname' , course_disc ='$cdisc' , course_author = '$cauthor' , course_duration = '$cduration' , course_price = '$cprice' , course_original_price = '$coriginalprice' , course_img = '$cimg' , WHERE course_id ='$cid'";
		if($conn->query(sql) == TRUE){
			$msg = '<div class="alert alert-success col-sm-6 ml-5 mt-2" role="alert" >UPDATEED SUCCESSFULLY</div>';
...
<div class="col-sm-6 mt-5 mx-3 jumbotron">
	...
	<form action="" method="POST" enctype="multipart/form-data">
		...
		<div class="form-group">
			<label for="course_disc">Course Description</label>
			<textarea class="form-control" id="course_disc" name="course_disc" rows="2"><?php if(isset($row['course-disc'])){echo $row['course_disc'];}   ?></textarea>
		</div>
		<div class="form-group">
